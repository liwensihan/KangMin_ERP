...
layui.define(['layer', 'laytpl', 'form', 'upload', 'util'], function(exports){
  ...
  //表单提交
  form.on('submit(oneFrom)', function(data){
	  console.log(data.elem) //被执行事件的元素DOM对象，一般为button对象
	  console.log(data.form) //被执行提交的form对象，一般在存在form标签时才会返回
	  console.log(data.field) //当前容器的全部表单字段，名值对形式：{name: value}
	  layer.confirm('确定提交吗！', function(){ 
		  	url="staff/updateUser.action";
  ...
  form.on('submit(twoFrom)', function(data){
	  console.log(data.elem) //被执行事件的元素DOM对象，一般为button对象
	  console.log(data.form) //被执行提交的form对象，一般在存在form标签时才会返回
	  console.log(data.field) //当前容器的全部表单字段，名值对形式：{name: value}
	  layer.confirm('确定提交吗！', function(){ 
		  	url="staff/updatePwd.action";
  ...
  form.verify({
	  staEmail:function(value, item){
		  var isv;
		  $.ajax({
			    url:"staff/getPhone.action",    //请求的url地址
			    dataType:"json",   //返回格式为json
			    async:false,//请求是否异步，默认为异步，这也是ajax重要特性
			    data:{staEmail:value},    //参数值
			    type:"post",   //请求方式
  	  ...
  	  ,nowpass:function(value, item){
		  var isva ;
		  $.ajax({
			    url:"staff/getPwd.action",    //请求的url地址
			    dataType:"json",   //返回格式为json
			    async:false,//请求是否异步，默认为异步，这也是ajax重要特性
			    data:{staPwd:value},    //参数值
			    type:"post",   //请求方式
			    success:function(req){
			    	if(req.mes=="no"){
			    		isva = 0;
